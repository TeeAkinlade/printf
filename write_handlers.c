 ...
 * Return: Number of chars printed.
 */
int write_number(int is_negative, int ind, char buffer[], int flags, int width, int pre, int size)
{
	...
	if (is_negative)
		...
		extra_ch = ' ';
	return (write_num(ind, buffer, flags, width, pre, length, padd, extra_ch));
}
